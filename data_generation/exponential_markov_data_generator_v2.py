...
def initialize_start_prob(num_states, max_nonzero_entries=None, init_v=None):
    sparsified = np.random.exponential(size=num_states)
    if max_nonzero_entries is not None:
        sparsification = np.zeros(num_states)
        idx = np.random.choice(a=np.arange(num_states), size=max_nonzero_entries, replace=False, p=init_v)
        sparsification[idx] = 1
        sparsified = sparsified * sparsification
...
def rand_init_prob_matrix(nrow, ncol, max_nonzero_entries_per_row=None, init_v=None):
    return np.array([initialize_start_prob(ncol, max_nonzero_entries_per_row, init_v) for r in range(nrow)])
...
def init_trans_matrix(number_dep_states, number_states_from, number_states_to, max_nonzero_entries_per_row=None, init_v=None):
    matrix = rand_init_prob_matrix(number_states_from ** number_dep_states, number_states_to, max_nonzero_entries_per_row, init_v)
    return matrix.reshape(np.append(np.full(number_dep_states, number_states_from), number_states_to))
...
GEN_VERSION = 5 # Shuffle + fix sparse and actually exponential
###############
    ...
    for idx, lag in enumerate([2**exp for exp in range(args.lag_min, args.lag_max + 1)]):
        ...
        with open(train_file_name, 'w') as train_file, open(test_file_name, 'w') as test_file:
            ...
            # Code specific for HMM generation.
                ...
                init_prior = np.random.exponential(size=args.vocab_size)
                init_prior = init_prior / sum(init_prior)

                # Initialize the transition matrix.
                transition_matrix = init_trans_matrix(dep, args.hidden_size, args.hidden_size)
                emission_matrix = rand_init_prob_matrix(args.hidden_size, args.vocab_size, args.max_nonzero_emission, init_prior)
